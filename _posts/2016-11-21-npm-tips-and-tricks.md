...
All Node.js developers have already used `npm` to install their favorite dependencies. However, far beyond the very famous `npm install`, npm offers a wide bench of available commands to ease your daily job. Most of them are unknown, so let's enlight them.
...
Yet, it won't save the dependency in your `package.json` file, and thus, your co-workers won't install `lodash` automatically at their next `npm install`. You can of course use the `--save` (or `--save-dev`) option, or be lazier and use their respective shortcuts: `-S` and `-D`:
...
**Production dependencies** (related to the `dependencies` property in `package.json`) are dependencies your project needs to work. For instance, you may find here `express` or `koa`.
...
**Optional dependencies** are another kind of dependencies stored in `optionalDependencies` property (you can install them using `--save-optional` flag). Imagine you want to give your users the ability to choose which rich text editor they can use. You can specify [tinyMCE](https://github.com/tinymce/tinymce) and [Quill](https://github.com/quilljs/quill) as optional dependencies. Then, in your code, just check which one is installed to enable features from one or the other:
...
Finally, the **Bundled Dependencies** (`bundledDependencies` property) is a particular type of dependencies. They behave like the normal production dependencies, except they are not installed from `npm`. This may be useful if you want to retrieve a private module without setting up a whole private repository. But, as we are using Git for all our projects, we can just use Git repositories instead.

## Using Git Repositories instead of npm Modules

Sometimes, you don't want to use `npm` modules directly. Either you are maintaining a private repository, or are waiting for a critical branch to be merged, you can simply use Git repository URL directly in your `package.json` file:
...
If you are using GitHub, dependency URL may even be simplfied to:
...
Sometimes, you may forget to include the `--save` or `--save-dev` option to your `npm install`, causing the *"but it works on my machine"* syndrom. To prevent it, just run a `prune` sometimes:

``` sh
npm prune
```

This command simply removes all dependencies that are not present in your `package.json` file. If you forget to save one of them, your project should not build anymore.

It may also be used in production. Imagine you are working on your deployment process, and that something broke. You are going to debug in production directly (even if you know it is bad). And you may sometimes make a standard `npm install` command instead of a `npm install --production` one. Not a big deal, but how can you clean up all unrequired development dependencies? Using the following command:
...
`npm` is shipped with two useful (yet quite unknown) commands: `npm home` and `npm repo`. These commands open respectively the project home page and project repository in your favorite web browser. This way, no need to google cumbersome `GitHub + [repository name]` anymore.
...
As you may have noticed during a failed production deployment, `npm` doesn't strictly fix your dependencies version at npm install. It just fixes the major version, leaving the minor one upgrading. It leads sometimes to several headaches, as JavaScript ecosystem is not really rigorous about [semver](http://semver.org/). We noticed it again when letting `jquery-ui` upgrade itself on one of our customer project:
...
There are three ways to define a dependency version in your `package.json` file:
...
We may first think to simply remove carrets and tildes from our `package.json` file. It may work, but it also may break. Indeed, as explained above, loose dependencies update themselves automatically without updating the `package.json` file. Hence, a dependency may be at version `1.2.7` while your `package.json` still refers to `~1.2.0` version.
...
Finally, just call this freshly installed script to update your `package.json` file:
